...
namespace lotto
{
	...
	public partial class GanadoresLoteria : Form
	{
		...
		void GanadoresLoteriaLoad(object sender, EventArgs e)
		{
			...
			//Si no hay ganadores se esconden las labels
		...
		{	
			...
			if(cont == 4){
				informacion.pote_resto_loteria = informacion.pote_resto_loteria+informacion.monto_total;
				informacion.monto_total = 0;
			}else{
				info.guardar_ganancias("loteria");
			}
